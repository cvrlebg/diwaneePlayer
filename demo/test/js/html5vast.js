  ...
  for(var i=0;i<media_files.length;i++){
    if(media_files[i].getAttribute('type')==options.media_type){
      //if((media_files[i].getAttribute('bitrate')>options.media_bitrate_min) && (media_files[i].getAttribute('bitrate')<options.media_bitrate_max)){
        console.log(i);
        console.dir(media_files[i]);
        if((/MSIE 9.0/).test(navigator.userAgent)) {
          obj_vast.media_file = media_files[i].text;
          console.log('ie 9');
          console.log(obj_vast.media_file);
       } else {
          console.log('other');
          obj_vast.media_file = media_files[i].textContent;
        }
...
function h5vPreRoll(video_player_id, obj_vast, options){
  ...
  var video_player_play = function(event) {
    ...
    if((/MSIE 9.0/).test(navigator.userAgent)) {
      console.log('in');
      /*while (video_player.firstChild) {
        console.dir(video_player.firstChild);
        video_player.removeChild(video_player.firstChild);
      }*/
      $('#' + video_player_id).html('');
      video_player.currentSrc = obj_vast.media_file;
    }
    //console.dir(video_player);
    $('#' + video_player_id).load();
    video_player.load();
...
function h5vGetCurrentSrc(video_player_id){
  if((/MSIE 9.0/).test(navigator.userAgent)) {
    return document.getElementById(video_player_id).getAttribute("src");
  } else {
